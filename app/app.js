(function () {
    ...
    app.controller("StoreController", ['$scope', '$http', 'cfpLoadingBar', function ($scope, $http, cfpLoadingBar) {
        cfpLoadingBar.start();
        $http.get("//dev.shopalyst.com/shopalyst-service/hackerearth002/1.0/products?limit=100").success(function (data) {
            $scope.products = data;
            });

        $scope.loadFeed = function (e) {
            cfpLoadingBar.start();
            $http.get("//dev.shopalyst.com/shopalyst-service/hackerearth002/1.0/products/"+e).success(function (data) {
